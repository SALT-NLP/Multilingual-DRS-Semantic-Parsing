...
class EncoderRNN(nn.Module):
    def __init__(self, word_size, word_dim, pretrain_size, pretrain_dim, pretrain_embeddings, lemma_size, lemma_dim, dptag_size, dptag_dim,
        ...
        if args.minusPos and args.minusDependency:
            ...
            self.embeds2input = nn.Linear(word_dim + pretrain_dim + lemma_dim + dptag_dim, input_dim)
        elif args.minusDependency:
            self.embeds2input = nn.Linear(word_dim + pretrain_dim + lemma_dim + postag_dim, input_dim)
        else:
            self.embeds2input = nn.Linear(word_dim + pretrain_dim + lemma_dim + postag_dim + dptag_dim, input_dim)
    ...
    def all_forward(self, tree, sentence,trainable=True):
        ...
        if args.minusPos and args.minusDependency:
            ...
            inputs = self.tanh(self.embeds2input(
                torch.cat((word_embedded, pretrain_embedded, lemma_embedded, dptag_embedded),
                          1))).view(1, -1)
            ...
            inputs = self.tanh(self.embeds2input(
                torch.cat((word_embedded, pretrain_embedded, lemma_embedded, dptag_embedded, postag_embedded),
                          1))).view(1, -1)
                ...
                if type(input) == types.NoneType:
                    ...
                    for item in input:
                        if item < self.tag_size:
                            List.append(
                                self.tag_embeds(torch.tensor([item], dtype=torch.long, device=device)).unsqueeze(1))
                        else:
...
def trainIters(trn_instances, dev_instances, tst_instances, encoder, decoder, print_every=100, evaluate_every=1000,
    ...
    for instance in tst_instances:
        ...
        tst_sentence_variable.append(instance[7])
        tst_sentence_variable.append(instance[9].to(device))
        tst_sentence_variable.append(instance[10])
        tst_rootTrees.append(instance[8])
    ...
    while True:
        ...
        if iter % (evaluate_every*len(trn_instances)) == 0:
            with torch.no_grad():
                ...
                evaluate(tst_sentence_variables, tst_rootTrees, encoder, decoder,
...
def test(dev_instances, tst_instances, encoder, decoder):
    ...
    evaluate(tst_sentence_variables, tst_rootTrees,encoder, decoder, tst_out_dir + "tst.drs")
...
def main():
    ...
    tst_instances = data2instance(tst_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info, (dptag_to_ix, 0),(postag_to_ix,0)],tst_gold_file)
    print("tst size: " + str(len(tst_instances)))

    maxLength=computeMaxLength(trn_instances,dev_instances,tst_instances)
