...
def data2instance(trn_data, ixes,outPutFile):
	...
	for one in trn_data:
		...
		for terms in one[4]:
			use+=[int(terms[2])]
		instances.append([])
		instances[-1].append(torch.tensor([get_from_ix(w, ixes[0][0], ixes[0][1])
										   for i,w in enumerate(one[0]) if i in use],dtype=torch.long, device=device))
		instances[-1].append(torch.tensor([get_from_ix(w, ixes[1][0], ixes[1][1])
										   for i,w in enumerate(one[1]) if i in use],dtype=torch.long, device=device))
		instances[-1].append(torch.tensor([get_from_ix(w, ixes[2][0], ixes[2][1])
										   for i,w in enumerate(one[2]) if i in use],dtype=torch.long, device=device))
