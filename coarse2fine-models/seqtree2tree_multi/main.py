...
class EncoderRNN(nn.Module):
    def __init__(self, word_size, word_dim, pretrain_size, pretrain_dim, pretrain_embeddings, lemma_size, lemma_dim, dptag_size, dptag_dim,
                 postag_size, postag_dim,input_dim, hidden_dim, n_layers=1,dropout_p=0.0):
        ...
        if args.minusDependency:
            self.embeds2inputTree = nn.Linear(2 * hidden_dim, input_dim)
        else:
            self.embeds2inputTree = nn.Linear(2 * hidden_dim + dptag_dim, input_dim)
    ...
    def all_forward(self, tree, sentence,out):
        for idx in range(tree.num_children):
            self.all_forward(tree.children[idx], sentence,out)
        ...
        if args.minusDependency:
            inputs = self.tanh(self.embeds2inputTree(out[tree.idx])).view(1,-1)
        else:
            inputs = self.tanh(self.embeds2inputTree(torch.cat((out[tree.idx], dptag_embedded), 1))).view(1, -1)

        tree.state = self.node_forward(inputs, child_c, child_h)
        return tree.state

    def forward(self,tree,sentence,hidden,train=True):
        ...
        postag_embedded=self.postag_embeds(sentence[4])
        ...
        if args.minusPos:
            embeds = self.tanh(self.embeds2input( pretrain_embedded )).view(
                len(sentence[0]), 1, -1)
        elif args.minusWordEmbedding:
            embeds = self.tanh(self.embeds2input(postag_embedded)).view(
                len(sentence[0]), 1, -1)
        else:
            embeds = self.tanh(self.embeds2input(
                torch.cat(( pretrain_embedded, postag_embedded), 1))).view(
                len(sentence[0]), 1, -1)
        outputLSTM, _ = self.lstm(embeds, hidden)
        state= self.all_forward(tree,sentence,outputLSTM)
    ...
    def forward_2(self, sentence_variable, inputs, hidden, encoder_output, least, train, mask_variable):
        if train:
            ...
            while True:
                ...
                if idx == 17:
                    #print(idchoice,self.tags_info.tag_size)
                    assert (idchoice >= self.tags_info.tag_size)
                if idchoice==0:
                    print(self.rel_mask_pool.encoder_input_size)
                    print(self.rel_mask_pool.tags_info.tag_size)
                    print(self.rel_mask_pool.prev_word,self.rel_mask_pool.link)
                    print(mask_variable)
                    print(total_score)
                    assert False
...
def train(sentence_variable, input_variables, gold_variables, mask_variables, rootTree, encoder, decoder, encoder_optimizer,
    ...
    encoder_output, encoder_hidden = encoder(rootTree, sentence_variable,encoder_hidden)
...
def decode(sentence_variable, rootTree, encoder, decoder):
    encoder_hidden = encoder.initHidden()
    encoder_output, encoder_hidden = encoder(rootTree, sentence_variable,encoder_hidden,train=False)
        ...
        for i in range(len(instance[3])):
            ...
            if idx == 4:
                ...
                for k in range(len(instance[4][p])):
                    ...
                    target=decoder.rel_mask_pool.get_step_mask(least, temp_idx)

                    temp_mask.append(target)
...
def main():
    ...
    #assert (len(pretrain_to_ix) == sum([int(pretrain_data[0][0]) for pretrain_data in pretrain_data_list]) + 1)
    ...
    print("postag dict size: ", len(postag_to_ix))
    ...
    encoder = EncoderRNN(len(word_to_ix), WORD_EMBEDDING_DIM, len(pretrain_to_ix), PRETRAIN_EMBEDDING_DIM, torch.tensor(pretrain_embeddings,dtype=torch.float, device=device),
                         len(lemma_to_ix), LEMMA_EMBEDDING_DIM, len(dptag_to_ix), DEPENDENCY_TAG_DIM,  len(postag_to_ix), POS_TAG_DIM, INPUT_DIM, ENCODER_HIDDEN_DIM,n_layers=ENCODER_LAYER, dropout_p=ENCODER_DROUPOUT_RATE).to(device)
