...
dev_gold_file="output_dev/dev.test"
...
class EncoderRNN(nn.Module):
    def __init__(self, word_size, word_dim, pretrain_size, pretrain_dim, pretrain_embeddings, lemma_size, lemma_dim, dptag_size, dptag_dim,
                 postag_size, postag_dim,input_dim, hidden_dim, dropout_p=0.0):
    ...
    def all_forward(self, tree, sentence,trainable):
        for idx in range(tree.num_children):
            self.all_forward(tree.children[idx], sentence,trainable)
...
def trainIters(trn_instances, dev_instances, tst_nl_instances, tst_de_instances,tst_it_instances, encoder, decoder, print_every=100, evaluate_every=1000,
    ...
    for instance in trn_instances:
        ...
        for i in range(len(instance[5])-1):
            ...
            if (idx >= 13 and idx < decoder.tags_info.p_rel_start) or idx >= decoder.tags_info.tag_size:
                idx = instance[5][i]
                idy = instance[5][i + 1]
                decoder.var_mask_pool.update(idx)
                if (idx >= 14 and idx <= 16) or (
    ...
    while True:
        ...
        if iter % (evaluate_every*len(trn_instances)) == 0:
            with torch.no_grad():
                ...
                dev_loss3 = 0.0
                if args.savemodel:
                    torch.save({"iter": iter, "idx": idx, "encoder": encoder.state_dict(), "decoder": decoder.state_dict(),
...
def evaluate(sentence_variables, rootTrees,encoder, decoder, path):
    ...
    for idx in range(len(sentence_variables)):
        ...
        #print('len:',len(sentence_variables[idx][1]),len(sentence_variables[idx][5]))
        assert(len(sentence_variables[idx][1])==len(sentence_variables[idx][5]))
        ...
        for i in range(len(structs)):
            if structs[i] < decoder.tags_info.tag_size and not structs[i] == 17:
                ...
                if flag == 0:
                    #print(len(sentence_variables[idx][5]),structs[i] - decoder.tags_info.tag_size)
                    output.append(sentence_variables[idx][5][structs[i] - decoder.tags_info.tag_size] + '(')
                else:
                    output[-1] = output[-1][:-1] + '~' + sentence_variables[idx][5][
                        structs[i] - decoder.tags_info.tag_size] + '('
                    flag = 0
            if (structs[i] >= 14 and structs[i] <= 16) or (
                    structs[i] >= decoder.tags_info.global_start and structs[i] < decoder.tags_info.p_rel_start) \
...
def main():
    ...
    for sentence, _, lemmas, posTags, dps, _, tags in trn_data:
        for word in sentence:
            if word not in word_to_ix and word not in [':','-']:
                word_to_ix[word] = len(word_to_ix)
        for lemma in lemmas:
            if lemma not in lemma_to_ix and lemma not in [':','-']:
    ...
    tst_nl_instances = data2instance(tst_nl_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info,
                                                   (dptag_to_ix, 0), (postag_to_ix, 0)], tst_nl_gold_file)
    ...
    encoder = EncoderRNN(len(word_to_ix), WORD_EMBEDDING_DIM, len(pretrain_to_ix), PRETRAIN_EMBEDDING_DIM, torch.tensor(pretrain_embeddings,dtype=torch.float, device=device),
                         len(lemma_to_ix), LEMMA_EMBEDDING_DIM, len(dptag_to_ix), DEPENDENCY_TAG_DIM, len(postag_to_ix), POS_TAG_DIM, INPUT_DIM, ENCODER_HIDDEN_DIM, dropout_p=ENCODER_DROUPOUT_RATE).to(device)
