...
from utils import computeMaxLength
from tag import Tag
...
tst_nl_file = "../PMB_multi/PMB_nl_v2/PMB/gold/nl_input.txt"
tst_de_file = "../PMB_multi/PMB_de_v2/PMB/gold/de_input.txt"
tst_it_file = "../PMB_multi/PMB_it_v2/PMB/gold/it_input.txt"
trn_gold_file="train.test"
dev_gold_file="output_dev/dev.gold"
tst_nl_gold_file="output_nl_tst/tst.gold"
tst_de_gold_file="output_de_tst/tst.gold"
tst_it_gold_file="output_it_tst/tst.gold"

pretrain_files = ["../PMB_multi/wiki.multi.en.vec.txt","../PMB_multi/wiki.multi.nl.vec.txt","../PMB_multi/wiki.multi.de.vec.txt","../PMB_multi/wiki.multi.it.vec.txt"]
tag_info_file = "../PMB2/gold/tag.txt"

dev_out_dir = "output_dev/"
tst_nl_out_dir = "output_nl_tst/"
tst_de_out_dir = "output_de_tst/"
tst_it_out_dir = "output_it_tst/"
...
PRETRAIN_EMBEDDING_DIM = 300
LEMMA_EMBEDDING_DIM = 32
DEPENDENCY_TAG_DIM = 32
POS_TAG_DIM=32
TAG_DIM = 128
INPUT_DIM = 100
...
def position_encoding_init(n_position, d_pos_vec):
    ...
    return torch.from_numpy(position_enc).type(dtype=torch.float)

class EncoderRNN(nn.Module):
    def __init__(self, word_size, word_dim, pretrain_size, pretrain_dim, pretrain_embeddings, lemma_size, lemma_dim, dptag_size, dptag_dim,
                 postag_size, postag_dim,input_dim, hidden_dim, max_length, dropout_p=0.0):
        ...
        self.dptag_embeds= nn.Embedding(dptag_size,dptag_dim)
        self.postag_embeds = nn.Embedding(postag_size, postag_dim)
        self.dropout = nn.Dropout(self.dropout_p)

        self.pos1_embeds=nn.Embedding(max_length,word_dim)
        self.pos2_embeds=nn.Embedding(max_length,pretrain_dim)
        self.pos3_embeds = nn.Embedding(max_length, lemma_dim)
        self.pos4_embeds = nn.Embedding(max_length, dptag_dim)
        self.pos1_embeds.weight=nn.Parameter(position_encoding_init(max_length,word_dim),False)
        self.pos2_embeds.weight = nn.Parameter(position_encoding_init(max_length, pretrain_dim), False)
        self.pos3_embeds.weight = nn.Parameter(position_encoding_init(max_length, lemma_dim), False)
        self.pos4_embeds.weight = nn.Parameter(position_encoding_init(max_length, dptag_dim), False)

        if args.minusPos and args.minusDependency:
            self.embeds2input = nn.Linear( pretrain_dim , input_dim)
        elif args.minusPos and args.minusWordEmbedding:
            self.embeds2input = nn.Linear(dptag_dim, input_dim)
            ...
            self.embeds2input = nn.Linear( postag_dim + dptag_dim, input_dim)
        elif args.minusPos:
            self.embeds2input = nn.Linear( pretrain_dim + dptag_dim, input_dim)
        elif args.minusDependency:
            self.embeds2input = nn.Linear( pretrain_dim + postag_dim, input_dim)
        else:
            self.embeds2input = nn.Linear( pretrain_dim + postag_dim + dptag_dim, input_dim)
    ...
    def all_forward(self, tree, sentence,trainable=True):
        for idx in range(tree.num_children):
            self.all_forward(tree.children[idx], sentence)

        if tree.num_children == 0:
            child_c = torch.zeros(1, self.mem_dim, dtype=torch.float, device=device)
            child_h = torch.zeros(1, self.mem_dim, dtype=torch.float, device=device)
        else:
            child_c, child_h = zip(*map(lambda x: x.state, tree.children))
            child_c, child_h = torch.cat(child_c, dim=0), torch.cat(child_h, dim=0)

        word_embedded = self.word_embeds(sentence[0].unsqueeze(0).transpose(0, 1)[tree.idx])
        pretrain_embedded = self.pretrain_embeds(sentence[1].unsqueeze(0).transpose(0, 1)[tree.idx])
        lemma_embedded = self.lemma_embeds(sentence[2].unsqueeze(0).transpose(0, 1)[tree.idx])
        postag_embedded = self.postag_embeds(sentence[4].unsqueeze(0).transpose(0, 1)[tree.idx])
        dptag_embedded=self.dptag_embeds(torch.tensor([tree.tag],dtype=torch.long, device=device))

        assert(tree.idx+1<self.max_length)
        word_embedded+=self.pos1_embeds(torch.tensor([tree.idx+1],dtype=torch.long, device=device))
        pretrain_embedded += self.pos2_embeds(torch.tensor([tree.idx + 1], dtype=torch.long, device=device))
        lemma_embedded += self.pos3_embeds(torch.tensor([tree.idx + 1], dtype=torch.long, device=device))
        dptag_embedded += self.pos4_embeds(torch.tensor([tree.idx + 1], dtype=torch.long, device=device))

        if trainable:
            word_embedded = self.dropout(word_embedded)
            lemma_embedded = self.dropout(lemma_embedded)
            dptag_embedded =self.dropout(dptag_embedded)
            postag_embedded = self.dropout(postag_embedded)

        if args.minusPos and args.minusDependency:
            inputs = self.tanh(self.embeds2input(pretrain_embedded)).view(1, -1)

        elif args.minusPos and args.minusWordEmbedding:
            inputs = self.tanh(self.embeds2input(dptag_embedded)).view(1, -1)
        elif args.minusDependency and args.minusWordEmbedding:
            inputs = self.tanh(self.embeds2input(postag_embedded)).view(1, -1)
        elif args.minusWordEmbedding:
            inputs = self.tanh(self.embeds2input(torch.cat((postag_embedded, dptag_embedded),1))).view(1, -1)


        elif args.minusPos:
            inputs = self.tanh(self.embeds2input(torch.cat(( pretrain_embedded, dptag_embedded), 1))).view(1, -1)

        elif args.minusDependency:
            inputs = self.tanh(self.embeds2input(torch.cat(( pretrain_embedded, postag_embedded),1))).view(1, -1)

        else:
            inputs = self.tanh(self.embeds2input(
                torch.cat(( pretrain_embedded, dptag_embedded, postag_embedded),1))).view(1, -1)

        tree.state = self.node_forward(inputs, child_c, child_h)
        return tree.state

    def forward(self,tree,sentence,train=True):
        state= self.all_forward(tree,sentence,trainable=train)
        output=torch.cat([itemTree.state[1].unsqueeze(0) for itemTree in sentence[3]])
        state=(state[1].unsqueeze(0),state[0].unsqueeze(0))
        return output,state
...
def train(sentence_variable, input_variables, gold_variables, mask_variables, rootTree, encoder, decoder, encoder_optimizer,
    ...
    encoder_output, encoder_hidden = encoder(rootTree, sentence_variable)

    ################structure
    decoder_hidden1 = encoder_hidden
    ...
    ################ relation
    decoder_hidden2 = encoder_hidden
    decoder_input2 = []
    ...
    ################ variable
    decoder_hidden3 = encoder_hidden
    decoder_input3 = []
...
def decode(sentence_variable, rootTree, encoder, decoder):
    encoder_output, encoder_hidden = encoder(rootTree, sentence_variable,train=False)
...
def trainIters(trn_instances, dev_instances, tst_nl_instances, tst_de_instances,tst_it_instances, encoder, decoder, print_every=100, evaluate_every=1000,
    ...
    for instance in trn_instances:
        ...
        sentence_variables[-1].append(instance[7])
        sentence_variables[-1].append(instance[9].to(device))
        sentence_variables[-1].append(instance[10])
        rootTrees.append(instance[8])
    ...
    for instance in dev_instances:
        ...
        dev_sentence_variables[-1].append(instance[7])
        dev_sentence_variables[-1].append(instance[9].to(device))
        dev_sentence_variables[-1].append(instance[10])
        dev_rootTrees.append(instance[8])
    ...
    # ====================================== test
    tst_nl_sentence_variables = []
    tst_nl_rootTrees = []

    for instance in tst_nl_instances:
        tst_sentence_nl_variable = []
        tst_sentence_nl_variable.append(instance[0].to(device))
        tst_sentence_nl_variable.append(instance[1].to(device))
        tst_sentence_nl_variable.append(instance[2].to(device))
        tst_sentence_nl_variable.append(instance[7])
        tst_sentence_nl_variable.append(instance[9].to(device))
        tst_sentence_nl_variable.append(instance[10])
        tst_nl_rootTrees.append(instance[8])

        tst_nl_sentence_variables.append(tst_sentence_nl_variable)

    tst_de_sentence_variables = []
    tst_de_rootTrees = []

    for instance in tst_de_instances:
        tst_sentence_de_variable = []
        tst_sentence_de_variable.append(instance[0].to(device))
        tst_sentence_de_variable.append(instance[1].to(device))
        tst_sentence_de_variable.append(instance[2].to(device))
        tst_sentence_de_variable.append(instance[7])
        tst_sentence_de_variable.append(instance[9].to(device))
        tst_sentence_de_variable.append(instance[10])
        tst_de_rootTrees.append(instance[8])

        tst_de_sentence_variables.append(tst_sentence_de_variable)

    tst_it_sentence_variables = []
    tst_it_rootTrees = []

    for instance in tst_it_instances:
        tst_sentence_it_variable = []
        tst_sentence_it_variable.append(instance[0].to(device))
        tst_sentence_it_variable.append(instance[1].to(device))
        tst_sentence_it_variable.append(instance[2].to(device))
        tst_sentence_it_variable.append(instance[7])
        tst_sentence_it_variable.append(instance[9].to(device))
        tst_sentence_it_variable.append(instance[10])
        tst_it_rootTrees.append(instance[8])

        tst_it_sentence_variables.append(tst_sentence_it_variable)
    ...
    while True:
        ...
        loss1, loss2, loss3 = train(sentence_variables[idx],
                                    ...
                                    (mask1_variables[idx], mask2_variables[idx], mask3_variables[idx]), rootTrees[idx],encoder,
            ...
            with torch.no_grad():
                ...
                if args.savemodel:
                    torch.save(
                        ...
                        model_dir + str(int(iter * 1.0 / len(trn_instances))) + ".model")
                ...
                while dev_idx < len(dev_instances):
                    ...
                    a, b, c = train(dev_sentence_variables[dev_idx], (
                        dev_input1_variables[dev_idx], dev_input2_variables[dev_idx], dev_input3_variables[dev_idx]), (
                                        ...
                                        dev_mask3_variables[dev_idx]), dev_rootTrees[dev_idx],encoder, decoder, encoder_optimizer,
                                    decoder_optimizer,
                ...
                print('epoch %.6f : dev loss %.10f, s1: %.10f, s2: %.10f, s3: %.10f ' % (
                    iter * 1.0 / len(trn_instances), dev_loss / len(dev_instances), dev_loss1 / len(dev_instances),
                    dev_loss2 / len(dev_instances),
                    dev_loss3 / len(dev_instances)))
                evaluate(dev_sentence_variables, dev_rootTrees, encoder, decoder,
                ...
                evaluate(tst_nl_sentence_variables, tst_nl_rootTrees, encoder, decoder,
                         tst_nl_out_dir + str(int(iter / len(trn_instances))) + ".drs")
                evaluate(tst_de_sentence_variables, tst_de_rootTrees, encoder, decoder,
                         tst_de_out_dir + str(int(iter / len(trn_instances))) + ".drs")
                evaluate(tst_it_sentence_variables, tst_it_rootTrees, encoder, decoder,
                         tst_it_out_dir + str(int(iter / len(trn_instances))) + ".drs")


def evaluate(sentence_variables, rootTrees,encoder, decoder, path):
    out = open(path, "w",encoding='utf-8')
    for idx in range(len(sentence_variables)):
        if use_cuda:
            torch.cuda.empty_cache()

        structs, tokens = decode(sentence_variables[idx], rootTrees[idx],encoder, decoder)
...
def test(dev_instances,tst_nl_instances, tst_de_instances,tst_it_instances, encoder, decoder):
    # ====================================== test
    dev_sentence_variables = []
    dev_rootTrees=[]
    ...
    for instance in dev_instances:
        ...
        dev_sentence_variable.append(instance[7])
        dev_sentence_variable.append(instance[9].to(device))
        dev_sentence_variable.append(instance[10])
        dev_rootTrees.append(instance[8])
    ...
    evaluate(dev_sentence_variables, dev_rootTrees,encoder, decoder, dev_out_dir + "dev.drs")
    evaluate(tst_nl_sentence_variables, tst_nl_rootTrees,encoder, decoder, tst_nl_out_dir + "tst.drs")
    evaluate(tst_de_sentence_variables, tst_de_rootTrees, encoder, decoder, tst_de_out_dir + "tst.drs")
    evaluate(tst_it_sentence_variables, tst_it_rootTrees, encoder, decoder, tst_it_out_dir + "tst.drs")
...
def main():
    ...
    for sentence, _, lemmas, posTags, dps, _, tags in trn_data:
        ...
        for term in dps:
            if term[0] not in dptag_to_ix:
                dptag_to_ix[term[0]]=len(dptag_to_ix)
    ...
    pretrain_data_list = readpretrain(pretrain_files)

    for pretrain_data in pretrain_data_list:
        for one in pretrain_data[1:]:
            if one[0] not in pretrain_to_ix:
                pretrain_to_ix[one[0]] = len(pretrain_to_ix)
                pretrain_embeddings.append([float(a) for a in one[1:]])

    print(sum([int(pretrain_data[0][0]) for pretrain_data in pretrain_data_list]),len(pretrain_to_ix))

    assert (len(pretrain_embeddings[0]) == int(pretrain_data_list[0][0][1]))
    assert (len(pretrain_embeddings) == len(pretrain_to_ix))
    print("pretrain dict size:", len(pretrain_to_ix))

    dev_data = readfile(dev_file)
    tst_nl_data = readfile(tst_nl_file)
    tst_de_data = readfile(tst_de_file)
    tst_it_data = readfile(tst_it_file)

    print("word dict size: ", len(word_to_ix))
    print("lemma dict size: ", len(lemma_to_ix))
    print("dptag dict size: ", len(dptag_to_ix))
    ...
    trn_instances = data2instance(trn_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info, (dptag_to_ix, 0),(postag_to_ix,0)],trn_gold_file)
    print("trn size: " + str(len(trn_instances)))
    ###########################################################
    # prepare development instance
    dev_instances = data2instance(dev_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info, (dptag_to_ix, 0),(postag_to_ix,0)],dev_gold_file)
    print("dev size: " + str(len(dev_instances)))
    ###########################################################
    # prepare test instance
    tst_nl_instances = data2instance(tst_nl_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info, (dptag_to_ix, 0),(postag_to_ix,0)],tst_nl_gold_file)
    print("tst nl size: " + str(len(tst_nl_instances)))
    tst_de_instances = data2instance(tst_de_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info,
                                                   (dptag_to_ix, 0), (postag_to_ix, 0)], tst_de_gold_file)
    print("tst de size: " + str(len(tst_de_instances)))
    tst_it_instances = data2instance(tst_it_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info,
                                                   (dptag_to_ix, 0), (postag_to_ix, 0)], tst_it_gold_file)
    print("tst it size: " + str(len(tst_it_instances)))

    maxLength=computeMaxLength([trn_instances,dev_instances,tst_nl_instances,tst_de_instances,tst_it_instances])
    print('maxLength:',maxLength)
    encoder = EncoderRNN(len(word_to_ix), WORD_EMBEDDING_DIM, len(pretrain_to_ix), PRETRAIN_EMBEDDING_DIM, torch.tensor(pretrain_embeddings,dtype=torch.float, device=device),
                         len(lemma_to_ix), LEMMA_EMBEDDING_DIM, len(dptag_to_ix), DEPENDENCY_TAG_DIM, len(postag_to_ix), POS_TAG_DIM, INPUT_DIM, ENCODER_HIDDEN_DIM, maxLength, dropout_p=ENCODER_DROUPOUT_RATE).to(device)
    attn_decoder = AttnDecoderRNN(outer_mask_pool, rel_mask_pool, var_mask_pool, tags_info, TAG_DIM, DECODER_INPUT_DIM, ENCODER_HIDDEN_DIM, ATTENTION_HIDDEN_DIM, n_layers=DECODER_LAYER, dropout_p=DECODER_DROPOUT_RATE).to(device)

    if args.test:
        test(dev_instances, tst_nl_instances, tst_de_instances,tst_it_instances, encoder, attn_decoder)
    else:
        writer = SummaryWriter(log_dir)
        trainIters(trn_instances, dev_instances, tst_nl_instances, tst_de_instances,tst_it_instances,encoder, attn_decoder, print_every=PRINT_EVERY,
