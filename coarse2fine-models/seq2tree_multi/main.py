...
class EncoderRNN(nn.Module):
    def __init__(self, word_size, word_dim, pretrain_size, pretrain_dim, pretrain_embeddings, lemma_size, lemma_dim,
        ...
        if args.minusPos and args.minusDependency:
            ...
            self.embeds2input = nn.Linear( pretrain_dim + deptag_dim, input_dim)
        elif args.minusDependency:
            self.embeds2input = nn.Linear( pretrain_dim + postag_dim, input_dim)
        else:
            self.embeds2input = nn.Linear(pretrain_dim  + postag_dim + deptag_dim, input_dim)
    ...
    def forward(self, sentence, hidden, train=True):
        ...
        if args.minusPos and args.minusDependency:
            embeds = self.tanh(self.embeds2input(pretrain_embedded)).view(len(sentence[0]), 1, -1)
        elif args.minusPos and args.minusWordEmbedding:
            ...
            embeds = self.tanh(self.embeds2input(
                torch.cat((pretrain_embedded, deptag_embedded),1))).view(len(sentence[0]), 1, -1)
        elif args.minusDependency:
            embeds = self.tanh(self.embeds2input(
                torch.cat((pretrain_embedded, postag_embedded),1))).view(len(sentence[0]), 1, -1)

        else:
            embeds = self.tanh(self.embeds2input(torch.cat((pretrain_embedded, postag_embedded,deptag_embedded), 1))).view(len(sentence[0]), 1, -1)
...
def trainIters(trn_instances, dev_instances, tst_nl_instances, tst_de_instances,tst_it_instances, encoder, decoder, print_every=100, evaluate_every=1000,
    ...
    for instance in tst_nl_instances:
        ...
        tst_sentence_nl_variable.append(instance[7].to(device))
        tst_sentence_nl_variable.append(instance[8].to(device))
        tst_sentence_nl_variable.append(instance[9])
    ...
    for instance in tst_de_instances:
        ...
        tst_sentence_de_variable.append(instance[7].to(device))
        tst_sentence_de_variable.append(instance[8].to(device))
        tst_sentence_de_variable.append(instance[9])
    ...
    for instance in tst_it_instances:
        ...
        tst_sentence_it_variable.append(instance[7].to(device))
        tst_sentence_it_variable.append(instance[8].to(device))
        tst_sentence_it_variable.append(instance[9])
    ...
    while True:
        ...
        if iter % (evaluate_every*len(trn_instances)) == 0:
            with torch.no_grad():
                ...
                evaluate(tst_nl_sentence_variables,  encoder, decoder,
                         tst_nl_out_dir + str(int(iter / len(trn_instances))) + ".drs")
                evaluate(tst_de_sentence_variables,  encoder, decoder,
                         tst_de_out_dir + str(int(iter / len(trn_instances))) + ".drs")
                evaluate(tst_it_sentence_variables,  encoder, decoder,
...
def test(dev_instances, tst_nl_instances, tst_de_instances,tst_it_instances, encoder, decoder):
    ...
    evaluate(tst_nl_sentence_variables, encoder, decoder, tst_nl_out_dir + "tst.drs")
    evaluate(tst_de_sentence_variables, encoder, decoder, tst_de_out_dir + "tst.drs")
    evaluate(tst_it_sentence_variables, encoder, decoder, tst_it_out_dir + "tst.drs")
#####################################################################################
...
def main():
    ...
    print(len(pretrain_embeddings[0]),pretrain_data_list[0][0][1])
    ...
    tst_nl_instances = data2instance(tst_nl_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info,
                                                   (postag_to_ix, 0), (dep_to_ix, 0)], tst_nl_gold_file)
    ...
    tst_de_instances = data2instance(tst_de_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info,
                                                   (postag_to_ix, 0), (dep_to_ix, 0)], tst_de_gold_file)
    ...
    tst_it_instances = data2instance(tst_it_data, [(word_to_ix, 0), (pretrain_to_ix, 0), (lemma_to_ix, 0), tags_info,
                                                   (postag_to_ix, 0), (dep_to_ix, 0)], tst_it_gold_file)
