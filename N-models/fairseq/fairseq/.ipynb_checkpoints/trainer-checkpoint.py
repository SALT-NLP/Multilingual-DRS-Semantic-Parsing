...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        params = [[k[1] for k in chain(self.model.named_parameters(), self.criterion.parameters()) if k[0].startswith('encoder')], 
        [k[1] for k in chain(self.model.named_parameters(), self.criterion.parameters()) if not k[0].startswith('encoder')]]
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                own_state.update({k.replace('decoder.sentence_', ''): v for k, v in state["model"].items() if not 'decoder.lm_head' in k})
    ...
    def lr_step_update(self):
        ...
        metrics.log_scalar("lr_enc", new_lr[0], weight=0, priority=300)
        metrics.log_scalar("lr_dec", new_lr[1], weight=0, priority=300)
        return new_lr
