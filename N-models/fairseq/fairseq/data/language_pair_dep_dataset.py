...
def collate(
    ...
    src_dep_tokens = merge(
        'source_dep', left_pad=left_pad_source,
        pad_to_length=pad_to_length['source'] if pad_to_length is not None else None
    )
    ...
    src_dep_tokens = src_dep_tokens.index_select(0, sort_order)
...
class LanguagePairDepDataset(FairseqDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        assert(self.src_sizes[index]*2==self.src[index].size()[0])
        src_item = self.src[index][:self.src_sizes[index]]
        src_dep_item = self.src[index][self.src_sizes[index]:]
        ...
        if self.remove_eos_from_source:
            ...
            if src_item[-1] == eos:
                src_item = src_item[:-1]
                src_dep_item = src_dep_item[index][:-1]

        example = {
            'id': index,
            'source': src_item,
            'source_dep': src_dep_item,
            'target': tgt_item,
        }
        if self.align_dataset is not None:
