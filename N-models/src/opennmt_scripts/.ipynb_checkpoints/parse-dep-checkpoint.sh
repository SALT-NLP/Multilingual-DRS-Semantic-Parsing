...
if [ ! -z "$2" ] && [ ! -z "$3" ] && [ ! -z "$4" ] && [ ! -z "$5" ] && [ ! -z "$6" ] && [ ! -z "$7" ]; then
	...
	# Postprocess
        python $POSTPROCESS_PY --input_file $3 --input_sent_file $5 --input_lex_file $4 --output_file $3$res_ext --var $var_rewrite $no_sep --sig_file $sig_file > $3${res_ext}.log
        python counter2.py -f1 $3$res_ext -f2 $6 -pr>> $3${res_ext}.counter.log
