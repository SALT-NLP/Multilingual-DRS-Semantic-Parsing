...
from uts import write_list_of_lists, between_quotes, is_concept, get_first_arg_boxes
...
op_boxes = ['NOT','NEC','POS','OR','DUPLEX','IMP','RESULTS','CONTINUATION','EXPLANATION','CONTRAST']
...
def create_arg_parser():
    ...
    parser.add_argument("-is", "--input_sent_file", required=True, type=str, help="Input file with raw sents")
    parser.add_argument("-il", "--input_lex_file", required=True, type=str, help="Input file with raw lexicon sents")
...
class RestoreVariables:
    ...
    def __init__(self, drs, pp_info):
        ...
        self.var_list = self.get_misc_order()
        ...
        for clause in self.drs:
            for item in clause.split():
                if item == self.new_box:
                    box_order.append(self.box_char + str(len(box_order)))
        return box_order
    
    def get_misc_order(self):
        '''Number and order of boxes for relative rewriting'''
        misc_order = []
        for clause in self.drs:
            for item in clause.split():
                if item == self.new_var:
                    misc_order.append(self.disc_char + str(len(misc_order)))
        return misc_order
    ...
    def rewrite_length_two_three(self, cur_clause):
        ...
        if cur_clause[1] == self.ref and self.var_type == "rel":
            ...
            second_var = self.get_variable(cur_clause[0], False)
            self.new_clauses.append([first_var, self.ref, second_var])
        elif cur_clause[2][0] == "$":
    ...
    def rewrite_length_four(self, cur_clause):
        ...
        if not between_quotes(cur_clause[2]):
            second_var = self.get_variable(cur_clause[2], cur_clause[2][0] == "$")
        if not between_quotes(cur_clause[3]):
            third_var = self.get_variable(cur_clause[3], cur_clause[3][0] == "$")
...
def easy_fixes(drs, sent, lex_sent, pp_info):
    ...
    try:
        ...
        pp_info.pp_dict["dummies-pp"].append(pp_info.cur_idx)
    if not (len(sent) == len(lex_sent)):
        print(sent)
        print(lex_sent)
    #assert(len(sent) == len(lex_sent))

    lex_dic={}
    for t,l in zip(sent, lex_sent):
        lex_dic[t] = l
    for c in drs:
        if c[1] in lex_dic:
            c[1] = lex_dic[c[1]]
...
def do_postprocess(args):
    ...
    raw_sents = []
    raw_lexs = []
    with open(args.input_sent_file) as reader:
        for line in reader:
            raw_sents.append(line.strip().split())
    with open(args.input_lex_file) as reader:
        for line in reader:
            raw_lexs.append(line.strip().split())
    assert(len(raw_lexs)==len(pp_info.lines))
    assert(len(raw_sents)==len(pp_info.lines))

    drss = []
    # Each line is a DRS, loop over them here one-by-one
    for idx, (line, sent, lex_sent) in enumerate(zip(pp_info.lines, raw_sents, raw_lexs)):
        pp_info.cur_idx = idx
        ...
        drs = easy_fixes(drs, sent, lex_sent, pp_info)
